...
def vigenere_cipher(message, key):
    ...
    for char in message:
        kchar = key[key_index % key_length]
        if char == " ":
            result.append(" ")
        else:
            msg_pos = alphabet.index(char)
            key_pos = alphabet.index(kchar)
            result.append(alphabet[(msg_pos + key_pos) % 26])
        key_index += 1  
...
def scytale_cipher(message, shift):
    ...
    for i in range(len(message)):
        raw_index = (i // shift) + rows * (i % shift)
